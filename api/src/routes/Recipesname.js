...
app.get("/", async function (req, res) {
  ...
  if (name) {
    ...
    await axios
      .get(
        `https://api.spoonacular.com/recipes/complexSearch?query=${name}${key}&addRecipeInformation=true`
      )
      .then((respuesta) =>
        respuesta.data.results.map((recipe) => found.push(recipe))
      )
    ...
    await axios
      .get(
        `https://api.spoonacular.com/recipes/complexSearch?&number=100${key}&addRecipeInformation=true`
      )
      .then((respuesta) =>
        respuesta.data.results.map((recipe) => found.push(recipe))
      )
